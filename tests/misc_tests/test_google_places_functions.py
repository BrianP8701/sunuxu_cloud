...
class TestGooglePLacesFunctions(unittest.TestCase):
    def test_address_autocomplete(self):
        ...
        data = requests.get(get_function_url(f"address_autocomplete/{query}/{session_token}")).json()
        print(data)
    ...
    def test_get_place_details(self):
        ...
        data = requests.get(get_function_url(f"get_place_details/{place_id}/{session_token}")).json()
        print(data)
