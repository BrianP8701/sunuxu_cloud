...
async def test_person_orm():
    ...
    person = await db.read(PersonModel, person.id)
    ...
    person_row = await db.read(PersonRowModel, person_row.id)
    assert person_row.name == "John Doe Smith"
    assert person_row.type == "lead"
    assert person_row.active is True
    await db.delete(PersonModel, {"id": person.id})
    # Ensure delete cascade works
    person_row_exists = await db.exists(PersonRowModel, {"id": person.id})
...
async def test_property_orm():
    ...
    retrieved_property = await db.read(PropertyModel, new_property.id)
    ...
    updated_property = await db.read(PropertyModel, new_property.id)
    assert updated_property.notes == "Updated note"

    # Delete the property
    await db.delete(PropertyModel, {"id": new_property.id})

    deleted_property = await db.read(PropertyModel, new_property.id)
...
async def test_transaction_orm():
    ...
    retrieved_transaction = await db.read(DealModel, new_transaction.id)
    ...
    updated_transaction = await db.read(DealModel, new_transaction.id)
    assert updated_transaction.notes == "Updated transaction note"

    # Delete the transaction
    await db.delete(DealModel, {"id": new_transaction.id})

    deleted_transaction = await db.read(DealModel, new_transaction.id)
