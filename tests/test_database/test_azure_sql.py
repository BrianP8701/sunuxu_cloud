...
async def test_insert_query_and_delete(db_instance):
    ...
    users = await db_instance.query(UserRowModel, {"email": "john@example.com"})
    assert len(users) == 1
    assert users[0].email == "john@example.com"
    await db_instance.delete(UserRowModel, {"id": inserted_user.id})
...
async def test_update(db_instance):
    ...
    users = await db_instance.query(UserRowModel, {"id": inserted_user.id})
    assert len(users) == 1
...
async def test_transactions(db_instance):
    ...
    try:
        ...
        users = await db_instance.query(
            UserRowModel, {"email": "transaction@example.com"}
        )
        assert len(users) == 1
    finally:
        await db_instance.delete(UserRowModel, {"email": "transaction@example.com"})
...
async def test_exists(db_instance):
    ...
    try:
        ...
        exists = await db_instance.exists(UserRowModel, {"email": "exists@example.com"})
        assert exists

        # Check if a non-existing user exists
        not_exists = await db_instance.exists(
            UserRowModel, {"email": "non_existing_user@example.com"}
        )
        assert not not_exists
    finally:
        await db_instance.delete(UserRowModel, {"email": "exists@example.com"})
