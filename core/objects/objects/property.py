...
class Property(BaseObject):
    ...
    property_orm: Optional[PropertyRowOrm]
    property_details_orm: Optional[PropertyOrm]
    deals: Optional[List[DealRow]]

    @classmethod
    async def read(cls, id: int):
        db = Database()
        property_details = await db.read(PropertyRowOrm, id, eager_load=[
            'property',
            'users',
            'owners',
            'occupants',
        ...
        occupants = [PersonRow.from_orm(occupant) for occupant in property_details.occupants]
    ...
    async def update(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if hasattr(PropertyRowOrm, key):
                property_updates[key] = value
            if hasattr(PropertyOrm, key):
                property_details_updates[key] = value

        async with db.get_session() as session:
            if property_updates:
                await db.update_fields(PropertyRowOrm, self.id, property_updates, session)
            if property_details_updates:
                await db.update_fields(PropertyOrm, self.id, property_details_updates, session)

    @classmethod
    async def delete(cls, id: int):
        db = Database()
        await db.delete_by_id(PropertyRowOrm, id)

    async def add_owner(self, person_id: int):
        db = Database()
        property = self.property_orm
        person = await db.read(PersonRowOrm, person_id)
    ...
    def _assemble_orm(self):
        property_data = {
            'address': assemble_address(self.street_number, self.street_name, self.street_suffix, self.city, self.unit, self.state, self.zip_code, self.country),
            'mls_number': self.mls_number,
            'type': self.type,
            'custom_type': self.custom_type,
            'active': self.active,
        ...
        user_associations = [UserPropertyAssociation(user_id=user_id, property_id=self.id) for user_id in self.user_ids]
        property_details_data['users'] = user_associations

        return PropertyRowOrm(**property_data), PropertyOrm(**property_details_data)
